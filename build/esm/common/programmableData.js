    ...
    async build() {
        ...
        for (const { txId, start, length } of this.readRanges) {
            ...
            if (dataStartCache.has(txId)) {
                ...
                const txMeta = (await Utils.wrapError(this.irys.storageTransactions.getHeader(txId))).data;
                if (txMeta.ledgerId !== 0)
                    throw new Error(`Transaction ${txId} is not permanent (ledger 0) and cannot be used.`);
                dataStart = await Utils.wrapError(await this.irys.storageTransactions.getLocalDataStartOffset(txId)).then((r) => BigInt(r.data.dataStartOffset));
